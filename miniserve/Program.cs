    ...
    public class Program {
        ...
        public static void Main(string[] args) {
            ...
            if (config.Port == 0) {
                ...
                const int minp = 62000;
                const int maxp = 63000;

                config.Port = (int) (random.NextDouble() * (maxp - minp) + minp);
            }
                    ...
                    foreach (var input in automation.SourceFiles) {
                        ...
                        if (!Directory.Exists(path)) {
                            pattern = path.Substring(
                                path.LastIndexOf("\\", StringComparison.InvariantCultureIgnoreCase) + 1);
            ...
            Process.Start(
                string.Format("http://localhost:{0}/", httpServer.Port),
                "");
        ...
        private static string ReplaceTags(string content) {
            ...
            foreach (var tag in tags) {
                ...
                switch (cc.Type) {
                    ...
                    case "js":
                        temp = string.Format("<script>{0}</script>", cc.Content);
                        break;

                    case "less":
                        temp = string.Format("<style type=\"text/css\">{0}</style>", cc.Content);
                        break;
                }
